    ...
    with bd.creer_connexion() as conn:
        ...
        if request.method == "POST":
            ...
            if contenu:
                bd.ajouter_message(
                    conn,
                    get_utilisateur_authentifié_or_die(),
                    identifiant,
                    contenu
                )
...
def afficher_amis(identifiant):
    ...
    with bd.creer_connexion() as conn:
        ...
        if "utilisateur" in session:
            id_authentifié = session["utilisateur"][0]   # pour accéder à l'identifiant stocké pendant l'authentification
...
def devenir_ami(identifiant):
    """Pour devenir ami"""
    id_authentifié = get_utilisateur_authentifié_or_die()
